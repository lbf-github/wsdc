	...
	grid = lyGrid({
		...
		l_column : [ {//表格列表数据
			...
			renderData : function(rowindex, data, rowdata, column) {
				if(data != ''){
					return '<img src="'+imgUrl+data+'" width="100px" height="100px"/>';
				}else{
...
function getCities(){
	...
	if (province_id != '' && province_id > 0){	
		...
		if (data != null) {
			...
			$("#city").append('<select id="city_id" name="activityFormMap.city_id" style="height:26" class="city_id"></select>');
			$(".city_id").html(h);
		 } else {
			layer.msg("获取数据出错，请联系管理员！");
		 }
...
function addFun() {
	pageii = layer.open({
		...
		type : 2,
		area : [ "600px", "80%" ],
		content : rootPath + '/activity/addUI.shtml'
	});
}
...
function editFun() {
	...
	pageii = layer.open({
		...
		type : 2,
		area : [ "600px", "80%" ],
		content : rootPath + '/activity/editUI.shtml?id=' + cbox
	});
}
