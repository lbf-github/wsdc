...
function getCities(){
	...
	if (province_id != '' && province_id > 0){	
		...
		if (data != null) {
			...
			$("#area").append('<select name="activityFormMap.city_id" class="form-control m-b city_id" tabindex="-1" required></select>');
			$(".city_id").html(h);
		 } else {
			layer.msg("获取数据出错，请联系管理员！");
		 }
...
function getAreas(){
	...
	if (city_id != '' && city_id > 0){	
		var url = rootPath + '/dataDic/findByPid.shtml?parentId='+city_id;
		var data = CommnUtil.ajax(url, null, "json");
		if (data != null) {
			var h = "<option value=''>-------请选择-----</option>";
			for ( var i = 0; i < data.length; i++) {
			...
			$("#area").append('<select name="leaseStationsFormMap.area_id" class="form-control m-b area_id" tabindex="-1" required></select>');
			$(".area_id").html(h);
		 } else {
			layer.msg("获取数据出错，请联系管理员！");
		 }
