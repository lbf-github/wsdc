...
public class LogAopAction {
	...
	// Controller层切点
	@Pointcut("@annotation(com.titi.annotation.SystemLog)")
	...
	public Object doController(ProceedingJoinPoint point) {
		...
		try {
			//ip = SecurityUtils.getSubject().getSession().getHost();
			ip = CusAccessObjectUtil.getIpAddress(request);
		} catch (Exception e) {
			ip = "无法获取登录用户Ip";
		}
		try {
		...
		try {
			if(map.get("methods") != null && map.get("description") != null){
